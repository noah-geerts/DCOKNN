...
//                    if (candidate_id == 0) continue;
        ...
        template <bool has_deletions, bool collect_metrics = false>
        std::priority_queue<std::pair<dist_t, tableint>, std::vector<std::pair<dist_t, tableint>>>
        searchBaseLayerAD(tableint ep_id, const void *data_point, size_t ef, Matrix<float> &magnitudes, float *err_sd, float q_mag, int adaptive) const
        {
            VisitedList *vl = visited_list_pool_->getFreeVisitedList();
            vl_type *visited_array = vl->mass;
            vl_type visited_array_tag = vl->curV;
                ...
                for (size_t j = 1; j <= size; j++)
                {
                    ...
                    if (!(visited_array[candidate_id] == visited_array_tag))
                    {
                        ...
                        else
                        {
                            ...
                            dist_t dist;
                            if (adaptive == 2)
                                dist = adsampling::dist_comp(lowerBound, getDataByInternalId(candidate_id), data_point, 0, 0);
                            else if (adaptive == 3)
                                dist = adsampling::dist_comp_pca(lowerBound, getDataByInternalId(candidate_id), data_point, err_sd, magnitudes.data[candidate_id] + q_mag);
                            else if (adaptive == 4)
                                dist = adsampling::dist_comp_adaptive_pca(lowerBound, getDataByInternalId(candidate_id), data_point, err_sd, magnitudes.data[candidate_id] + q_mag);
        ...
        searchKnn(void *query_data, size_t k, int adaptive, Matrix<float> &magnitudes, float *err_sd, float q_mag) const
        {
                    ...
                    for (int i = 0; i < size; i++)
                    {
                        ...
                        if (adaptive)
                        {
                            ...
                            if (adaptive == 1 || adaptive == 2)
                                d = adsampling::dist_comp(curdist, getDataByInternalId(cand), query_data, 0, 0);
                            else if (adaptive == 3)
                            {
                                d = adsampling::dist_comp_pca(curdist, getDataByInternalId(cand), query_data, err_sd, magnitudes.data[cand] + q_mag);
                            }
                            else if (adaptive == 4)
                            {
                                d = adsampling::dist_comp_adaptive_pca(curdist, getDataByInternalId(cand), query_data, err_sd, magnitudes.data[cand] + q_mag);
                            }
#ifdef COUNT_DIST_TIME
                            adsampling::distance_time += stopw.getElapsedTimeMicro();
#endif
                            if (d > 0 && d < curdist)  // Only move if distance is positive AND better
                            {
                                curdist = d;
                                currObj = cand;
                                changed = true;
            ...
            if (num_deleted_)
            {
                ...
                else if (adaptive == 2 || adaptive == 3 || adaptive == 4)
                    top_candidates = searchBaseLayerAD<true, true>(currObj, query_data, std::max(ef_, k), magnitudes, err_sd, q_mag, adaptive);
                else
